...
`js/i18n.js` は実行時に `ja`・`en`・対象言語の3ファイルを非同期フェッチし、
`Object.assign({}, ja_dict, en_dict, lang_dict)` の順にマージします。
**対象言語にキーがなければ英語、英語にも無ければ日本語**にフォールバックします。
英語を挟むのは、トルコ語やビルマ語を選んだ閲覧者にとって日本語より英語のほうが
読める可能性が高いためです。`ja` と `en` は全キー揃っているので、全訳済みの言語では
表示は一切変わりません。新しいキーを追加するときの最低要件は `ja` と `en` への記載です。
...
| CALENDAR | `schedule.html` のインタラクティブカレンダー（イベントは `data/events.json` を実行時に fetch。追加・変更は main.js ではなく JSON を編集。8言語ラベル必須）。初期表示は閲覧者の**現在月**（イベントの範囲にクランプ） |
