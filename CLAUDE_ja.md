...
小牧市東部（篠岡地区）の学校再編計画に関する**ビルドステップなしの静的な地域情報メディア**です。
...
全体辞書 `data/i18n/<言語>.json` に落ちます。各ページが使うキーは 1266 のうち 9〜20% しかないため、
...
- **12言語すべてが全キー翻訳済み。**
  トルコ語（`tr`）・ビルマ語（`my`）は 2026-08-13 に、韓国語（`ko`）・
  ネパール語（`ne`）は 2026-09-18 に全キーが揃ったため、
  `i18n.js` の `PARTIAL` 配列は空で、「未翻訳部分は英語表示」の帯は出ない。
  `events.json` のイベントラベルは**12言語必須**（`auto_gates.py` の `LANGS`）。
  ポルトガル語はブラジル、スペイン語は中南米、中国語は中国大陸の語にそろえる
  （自動取得した見出しの翻訳も同じ基準。くわしくは CLAUDE.md）。
...
2. **機械検証ゲート**（`.github/scripts/auto_gates.py`）：編集範囲・スキーマ
   （イベント12言語必須）・外部リンク規則・**引用の実在照合**（創作検出）。
...
| CALENDAR | `schedule.html` のインタラクティブカレンダー（イベントは `data/events.json` を実行時に fetch。追加・変更は main.js ではなく JSON を編集。12言語ラベル必須）。初期表示は閲覧者の**現在月**で固定。予定が0件の月でもクランプせず今月を開く（開いた人がまず知りたいのは「今がどこか」のため）。予定のある月へ自動で飛ばさないこと |
...
- **ボタンに文字ラベルは付けず、アイコンだけ**。サービス名は `aria-label`
  ...
  増えるうえ、12言語で同じ幅に揃わないため。`share_lead` / `share_note` を
  1行の短文にしているのも同じ理由。
...
状況が動いたとき（行事の終了・新資料の公表・決定など）は `now_text` と `now_asof` を**全12言語＋ja-kids**そろえて更新する。誤字修正など状況が動かない変更では触らない。`data/site-facts.json` に `now_bar` として登録し `default_targets` にも入れてあるので、自動更新パイプラインも毎回ここを更新候補として扱う。
...
| `hreflang` | 全ページの `<head>` | 12言語＋`x-default`。それぞれ**別々の** `?lang=` URL を指す。以前は11本すべてが同一 URL を指しており、これは Search Console がエラーとして報告する状態だった。 |
