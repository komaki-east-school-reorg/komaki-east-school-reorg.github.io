...
- **出すのは2種類**（2026-09-22 ユーザー指示で `[内容]` を追加）。`[機能]` は画面の仕組み・操作・見せ方、`[内容]` は載せる中身そのもの。見出しにこの印を付けさせるので、`auto_ideas/history.json` にも印が残り、後から種類が分かる。**`[内容]` の提案には「どの資料のどこを根拠にするか」を必ず書かせる** — 書けない案は出させない。使ってよい出どころは `data/official_pages/` の取り込み済み資料・市の公表ページ・文科省（`nationwide.html` 限定）・対象8校のHP・行政以外の主体についてはその主体自身の公表で、サイト本体の規則と同じ。
