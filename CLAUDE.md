...
- **Q&A のカテゴリ見出しには `faq-cat` クラスが要る。** `section-title sub` だけだと h2 と同じ「左に黄色い帯・緑の太字」になり、すぐ上の `faq_qa_h` と見分けがつかず見出しが二重に見える（同日ユーザー指摘）。`.faq-cat` は左帯を消して下線の小見出しにするもので、**`sub` は外さないこと** — READ ALOUD が `h2.section-title, h3.section-title.sub` を対象にしているため、外すとカテゴリ単位の読み上げボタンが消える。
