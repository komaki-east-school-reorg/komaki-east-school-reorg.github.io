...
- `source_url` may only point at an account listed in `PERMITTED_INSTAGRAM` in `auto_gates.py`. That map is **account → the pages it may appear on**: `arigato.ohshirosho` and `tokadaidancefestival` are confined to `community.html` and `index.html` (the two pages that render this corner), while an account whose value is `None` may appear anywhere — that is what the footer links use. Adding an account, or another page that renders the corner, requires updating this file, `CONTRIBUTING.txt` rule 1, `README.md` and that gate together.
