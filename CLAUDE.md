...
The corner does **not** reach the 回覧板 sheet's 最新の動き mode: that mode keeps only items carrying `data-date` within the last 7 days, and these are future events with no `data-date`. That is deliberate — the sheet reports what has happened, not what is coming — so do not "fix" it by giving the action items a `data-date`.
