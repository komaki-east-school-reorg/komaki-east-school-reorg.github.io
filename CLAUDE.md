...
- **An entry without `date` is a standing effort, not an event** (the two 児童館 and バンブーインスタレーション, whose 2026 dates are not yet known — 2026-09-26). It never expires, is listed after the dated events, carries `data-standing` on its `.action-item`, and is **left out of the 回覧板 sheet's 「これからの催し」**. Nothing goes on the schedule for it; the schedule rule above applies once a date is known — **when バンブーインスタレーション announces its dates, add `date` and put it on the schedule**.
- **⛔ Instagram prohibits automated collection** (its `robots.txt`: no collection by automated means without written permission, and `ClaudeBot` is disallowed outright). Do not fetch Instagram with AI browsing tools or scripts, and do not build a workflow that scrapes these accounts. Their content is checked by a person — see the weekly reminder Issue below if one exists.
