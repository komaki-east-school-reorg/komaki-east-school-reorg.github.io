...
- If nothing renders within 4 s the star row (`.share-star-hatena`) hides itself, so a broken widget never leaves a label with no star beside it. It hides only the Hatena part — the Facebook Like button below it is independent.

**The Facebook Like button** sits under the star row, loaded by the same `IntersectionObserver`.

- It is the **iframe plugin (`plugins/like.php`), never the JavaScript SDK** (`connect.facebook.net/…/sdk.js`). The SDK would run Facebook's code in this site's own origin with access to the DOM and cookies; the iframe keeps everything inside facebook.com's origin, so what leaves is the URL and Facebook's own cookies. Do not "upgrade" it to the SDK.
- It is the heaviest privacy cost on the site: it reports the page URL to Facebook on display, not on click, for any reader with a Facebook session. That is why it is behind the same lazy load, and why `share_note` names it explicitly alongside Hatena Star. If the button is ever removed, take it out of `share_note` too.
- `href` is the **bare canonical URL with no `?lang=`**, same reasoning as the star. The `locale` parameter does follow the reader's language (`FB_LOCALE`), and the iframe is re-pointed only when the language actually changes, so switching language does not re-hit Facebook needlessly.
- The iframe's height is reserved in CSS (`.fb-like-frame`) so its arrival does not shove the footer down.
