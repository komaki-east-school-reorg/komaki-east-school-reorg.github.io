...
- **The 対象エリア (purple) is still a by-eye trace** of the vol.6 figure, now clipped to the district polygons above — the trace had ~2 km² spilling outside the district, into 大字本庄・東田中 which are not in the Shinooka district at all. Clipping removes trace error rather than adding any. The original figure is cut off at the north, so only **9.81 km² of the published 18.5 km²** bus area is drawn; uncoloured land inside the red line means "not in the original figure", not "not covered", and `bus_map_legend_walk` says so.
- **Clipping splits the purple area into 3 disjoint pieces**, so `js/main.js` holds `busPolys` as an array. Do not collapse it back to a single polygon.
- **The map frame comes from `view_bbox` at the top of the geojson, not from the data extent.** 桃花台東 continues past the north-east corner of the frame (the hills of 大字大山・野口); fitting all of it in shrinks the built-up part of 桃花台 by about a third, which is the part readers actually look at. The red line is therefore cut by the viewBox — that is intended, and `bus_map_caveat` / `bus_map_legend_line` both say the district continues beyond the frame. Both district labels must stay inside `view_bbox`. Remove `view_bbox` and `main.js` falls back to fitting everything.
