...
The **地域の取組 section near the bottom of `index.html`** lists things local residents are doing to mark the closing schools — currently 「ありがとう大城小」, a farewell event at Oshiro Elementary on 2026-12-13. Hand-maintained; rendered by the COMMUNITY ACTIONS block in `js/main.js`. Past events drop off the list automatically (`date` compared with today).
...
- `source_url` may only point at an account listed in `PERMITTED_INSTAGRAM` in `auto_gates.py`, and Instagram links are allowed on `index.html` only (`INSTAGRAM_PAGES`). Adding an account requires updating this file, `CONTRIBUTING.txt` rule 1, `README.md` and that gate together.
