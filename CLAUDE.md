...
- **The 桃花台東/西 boundary (red line) is placed on 県道荒井大草線, not traced**, from the north edge of 池之内・古雅 (lat ≈35.3075) down to just south of 城山一〜二丁目 (lat ≈35.2966). The evidence: of the 37 OSM 丁目/字 centroids in the district whose school assignment is known from `faq_a2` and `about_oshiro_box`, **36 fall on the district-correct side of that road**. The one exception is 城山三丁目 — west of the road but part of 桃花台東 — so that short stretch is interpolated from the 丁目 centroids instead. Everything else is still the by-eye trace of the newsletter figure and is off by up to ~300 m. `boundary_note` on each `district` feature records this and `bus_map_caveat` states it to the reader. If the boundary is ever re-traced, do not put the road stretch back.
- Traced areas after that correction: 桃花台東 12.77 km² (published 20.3, the figure is cut off at the north), 桃花台西 7.77 km² (published 6.9 — the ~13% overshoot is the remaining error in the *outer* edges, which are still traced). 高根三丁目 still falls ~50 m on the wrong side; that is in the traced part, not the road part.
