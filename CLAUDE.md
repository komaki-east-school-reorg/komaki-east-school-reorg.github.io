...
- **分類は「いまの学校名」で、再編後も永続的にこの分け方を使う**（ユーザー指示）。学校名の中はさらに種目で分ける。活動場所が特定の校区に結びつかないものだけ `#wide` に置く。**校区が確認できないものを推測で学校に割り当てないこと。** 光ヶ丘チェリーズは当初、名称に「光ヶ丘」を含むだけでは根拠にならないとして `#wide` に置いていたが、2026-09-22 にユーザーから活動場所が光ヶ丘小学校だと示され、光ヶ丘小学校の欄へ移した。**裏づけが得られたら移す、得られないうちは `#wide` に置く**、という順でよい。
