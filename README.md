...
1. **外部リンクは公式市ページの3つのインデックスのみ** — 学校再編 `.../kyoiku/kyouikusoumu/303/index.html`（サイト全体）、地域協議会 `.../kenkouikigai/sasaeai/3/3_2/index.html`（community.html のみ）、東部まちづくり `.../toubumachidukuri/tobumachidukurisingikai/index.html`（「地域の取組」欄の出典）。それ以外の city.komaki.aichi.jp URL は禁止。別枠として、再編対象8校のホームページ（`komaki-aic.ed.jp`）、報道コーナーが出典として張る中日新聞Web の記事（`chunichi.co.jp`）、nationwide.html が出典として張る文部科学省の4つの資料ページ（`mext.go.jp`）、「地域の取組」（community.html と、同じ一覧を出す index.html）が発信元として張る市民有志の許可アカウント（`instagram.com/arigato.ohshirosho`）、全ページのフッタ「参考リンク（住民有志）」が張る桃花台を考える会【新しいまちづくり】の3つのSNS（`instagram.com/tokadai_komaki`・`x.com/tokadai_komaki`・`facebook.com/TokadaiNT`）、そして全ページ下部の共有ボタンの送り先（LINE・X・Facebook・はてなブックマーク・Threads・Bluesky・Reddit・Mastodon／はてなスター。スマートフォンではアプリが入っていればアプリを開く）は可。いずれも `auto_gates.py` が機械検査する
